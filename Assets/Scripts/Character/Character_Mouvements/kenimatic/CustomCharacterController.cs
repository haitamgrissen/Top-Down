...
public class CustomCharacterController : MonoBehaviour, ICharacterController
{
    ...
    public float MaxWalkingMoveSpeed = 10f;
    public float walkingMovementSharpness = 15;
    public float WalkingOrientationSharpness = 10;

    [Header("runinng Movement")]
    public float MaxRunningMoveSpeed = 10f;
    public float RunningMovementSharpness = 15;
    public float RunningOrientationSharpness = 10;
    ...
    private void Awake()
    {
        //calculate garvity and jump vars
    ...
    /// This is called every frame by MyPlayer in order to tell the character what its inputs are
        public void SetInputs(ref PlayerCharacterInputstruct inputs)
        {
            ...
            switch (CurrentCharacterState)
            {
                case CharacterStates.Default:
                {
                    ...
                    _lookInputVector = _moveInputVector;//cameraPlanarDirection;
        ...
        public void UpdateRotation(ref Quaternion currentRotation, float deltaTime)
        {
            switch (CurrentCharacterState)
            {
                case CharacterStates.Default:
                {
                    if (_lookInputVector != Vector3.zero && WalkingOrientationSharpness > 0f)
                    {
                        // Smoothly interpolate from current to target look direction
                        Vector3 smoothedLookInputDirection = Vector3.Slerp(Motor.CharacterForward, _lookInputVector,
                            1 - Mathf.Exp(-WalkingOrientationSharpness * deltaTime)).normalized;
            ...
            case CharacterStates.running:
                {
                    if (_lookInputVector != Vector3.zero && RunningOrientationSharpness > 0f)
                    {
                        // Smoothly interpolate from current to target look direction
                        Vector3 smoothedLookInputDirection = Vector3.Slerp(Motor.CharacterForward, _lookInputVector,
                            1 - Mathf.Exp(-RunningOrientationSharpness * deltaTime)).normalized;
            ...
            Vector3 targetMovementVelocity = Vector3.zero;
                    if (Motor.GroundingStatus.IsStableOnGround)
                    {
                        ...
                        Vector3 reorientedInput = Vector3.Cross(Motor.GroundingStatus.GroundNormal,
                                                      ...
                                                      .normalized *
                                                  _moveInputVector.magnitude;
                        targetMovementVelocity = reorientedInput * MaxWalkingMoveSpeed;

                        // Smooth movement Velocity
                        currentVelocity = Vector3.Lerp(currentVelocity,
                            targetMovementVelocity,
                            1 - Mathf.Exp(-walkingMovementSharpness * deltaTime));
                    }
                    else
                    {
                        // Add move input
