...
public class Orbit_Camera : MonoBehaviour
{
	...
	{
		UpdateFocusPoint();
		regularCamera.orthographicSize = projectionSize;
	...
	{
		Vector2 input = inputVal;
			//new Vector2(Input.GetAxis("Camera Vertical"), Input.GetAxis("Camera Horizontal"));
	...
	{
		if (Time.unscaledTime - lastManualRotationTime < alignDelay || !useAutoRotate)
		{
			return false;
		}
